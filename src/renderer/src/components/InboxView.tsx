...
export default function InboxView(): React.ReactElement {
  ...
  const fetchEmails = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await window.api.email.fetchInbox()
      setEmails(result)
    } catch (err: unknown) {
          ...
          {emails.map((email) => (
            ...
            >
              <div className="flex items-start gap-2">
                {email.read ? (
                  <MailOpen size={14} className="text-muted mt-0.5 flex-shrink-0" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <p
                      className={`text-xs truncate ${
                        email.read ? 'text-white/60' : 'text-white font-medium'
                      }`}
                    >
                  ...
                  <p
                    className={`text-xs truncate ${email.read ? 'text-muted' : 'text-white/80'}`}
                  >
                    {email.subject}
                  </p>
            ...
            <div className="px-6 py-4 border-b border-border flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-sm text-muted">
                  <span className="text-white/70">From:</span> {selected.from}
                </p>
